...
public class BankManagmentSystem extends JFrame implements ActionListener {
	...
	// add the Jframe
	Bank2Frame frame1 = new Bank2Frame(this);
	...
	public void CreateCryptoAccount() {
		...
		createCryptoAccountFrame.setLayout(new GridLayout(9, 4, 6, 6));
		...
		JLabel cryptoBalanceLabel = new JLabel("Enter investment amount:");
	...
	public void actionPerformed(ActionEvent e) {
		try {
			...
			if (e.getSource() == checkingSubmitButton) {
				...
				checking.displayOverDraftLimit(); 
				bankCount++;
				// message says that account was created
				JOptionPane.showMessageDialog(null, "Account created successfully!");
			...
			if (e.getSource() == mutualFundSumitButton) {// checks if array is full
				...
				MutualFundAccount mutualFund = new MutualFundAccount(accNum, name, email, phone, balance, investment,
						risk);
			...
			if (e.getSource() == cryptoSubmitButton) {// checks if array is full
				...
				CryptoAccount crypto = new CryptoAccount(accNum, name, email, phone, balance, cryptoBalance,
						cryptoType);
				bankAccounts[bankCount] = crypto;
			...
			JOptionPane.showMessageDialog(null, "Invalid input detected. Please check your entries.");
		}
	}

	public void viewAllBankInfo() {
	    if (bankCount == 0) {
	        JOptionPane.showMessageDialog(null, "No Bank accounts entered.");
	        return;
	    }

	    System.out.println("Total accounts created: " + bankCount);
	    System.out.println("Total slots available: " + bankAccounts.length);

	    for (int i = 0; i < bankAccounts.length; i++) { // Loop through the full array length
	        if (bankAccounts[i] != null) {
	            System.out.println("Account #" + i + ":");
	            bankAccounts[i].displayAccountInfo();
	        } else {
	            System.out.println("Account #" + i + " is NULL (not created).");
	        }
	    }
	}
	

	// Method to validate account number with try-catch
	private boolean isValidAccountNumber(String accNum) {
		try {
			if (accNum.length() == 0) {
				return false;
			}
	...
	// Method to validate phone number with try-catch
	...
	// Method to validate email with try-catch
	...
	// method to check if the phone number contains only numbers
	public static boolean isNumeric(String str) {
		try {
			...
			// Handle unexpected exceptions (e.g., null strings)
			return false;
		}
	}
	...
	public static String formatPhoneNumber(String phone) {
		try {
			...
			// If something goes wrong, just return the original phone number
			System.out.println("Error formatting phone number: " + e.getMessage());
			return phone;
		}
	}
}
